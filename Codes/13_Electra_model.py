...
class BertModel:
    def __init__(self,label):
        ...
        self.model = TFElectraForSequenceClassification.from_pretrained('google/electra-small-discriminator', num_labels = self.num_class)
    ...
    def run_model(self):
        ...
        for i in range(10):
            ...
            history = self.model.fit(train, train_y, batch_size = 16, epochs = 5, validation_data = (test,test_y), callbacks = callbacks.ReduceLROnPlateau(monitor='loss', factor=.2, patience=3, verbose=0, min_delta=1e-6, mode='min'))
